...
export class DependenciesStore extends ComponentStore<dependenciesState> {
    ...
    readonly getDependencies = this.effect<{ page: number; size: number; sortBy: string }>(
        trigger$ =>
            trigger$.pipe(
                tap(() => this.patchState({ loading: true, error: null })),
                switchMap(({ page, size, sortBy }) =>
                    this.sharedservice.getData<ApiResponse<any[]>>(`Account?page=${page}&size=${size}&sortBy=${sortBy}`).pipe(
                        tapResponse(
                            (accounts) => {
